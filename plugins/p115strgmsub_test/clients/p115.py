...
class P115ClientManager:
    ...
    def check_login(self) -> bool:
        ...
        try:
            ...
            # 检查是否有用户ID（某些版本API不返回state字段）
            user_id = user_info.get('user_id') or user_info.get('data', {}).get('user_id')
            if user_id:
                logger.info(f"115 登录成功: user_id={user_id}")
                return True
            ...
            try:
                ...
                if info and (info.get('user_id') or info.get('data', {}).get('user_id')):
                    logger.info("115 登录成功: 通过 user_info 验证")
                    return True
            except Exception:
                pass
            
            logger.error(f"115 登录验证失败，返回数据: {user_info}")
            return False
        except Exception as e:
